...
| 命令 | 结果 | 说明 |
| --- | --- | --- |
| `uv run pytest tests/test_utils.py -q` | `3 passed` | 覆盖 MimIR dump utility（跳过 default compile 优化测试防 crash）。 |
| `uv run pytest tests/test_basic.py -q` | `95 passed, 1 skipped` | 当前全部核心算子测试通过。 |
...
| kBroadcast 算子 | 支持了 `expand` 和 `full`。`full` 通过 `tensor.map` (ni=0) 构造，`expand` 基于 `tensor.broadcast_in_dim`。 | `src/mimir_frontend/operators.py`, `src/mimir_frontend/translator.py` |
